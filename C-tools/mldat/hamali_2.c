...
int hamali_2(unsigned long tp[][2], unsigned long *ts, unsigned long pp[][2],unsigned long *ps,int nch){
	int i,k;
	long f,g;
	...
	for(i=0;i<nch;i++){
		// temperatures
		if(tp[i][1]>tp[i][0]){      // need to know which is larger
			f=tp[i][1]-tp[i][0];
		}
		else{
			f=tp[i][0]-tp[i][1];
		...
		if (pp[i][0] != 0) {
			g=tp[i][0]/256L;
			if(f > g){
				ts[i]=tp[i][1];			// save for log
				tp[i][1]=tp[i][0];		// replace with first
				k++;
			}
		}
