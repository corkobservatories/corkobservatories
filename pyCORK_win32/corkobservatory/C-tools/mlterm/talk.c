...
void
talk (unsigned char *cmd, float seconds) {
	if (cmd) {
		writeDebugDelay(ifd,cmd,strlen((char *) cmd),"instrument", B9600);
		log_printf(LOG_SCREEN_MAX,"%s", cmd);
	}
	/* delay after sending command to comport */
	nap (seconds);
}
...
nap (float seconds) {
	...
	Debug("Start napping for %ld.%06ld seconds\n", delay_sec, delay_usec);
