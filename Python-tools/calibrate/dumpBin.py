...
def getStatistics(Data, loggerID=None,do_plots=False,interp_spikes=False):
    ...
    vData=Data[0:int(recLen*NRecs)].view()
...
#    quit()
    ...
    IdIdx=np.where(Data == loggerID)[0]
    #print recLen
    #print loggerID*np.ones(3*recLen, dtype=Data.dtype).shape
    #print Data.shape
    # Patch the data with a few records of logger IDs at the to simplify consistency checking
    Data=np.concatenate((Data,loggerID*np.ones(3*recLen, dtype=Data.dtype)),axis=0)
